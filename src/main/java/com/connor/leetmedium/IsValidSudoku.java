        ...
        for(int i = 0; i < board.length; i++) {
            for(int j = 0; j < board[0].length; j++) {
                if(board[i][j] != '.') {
                    ...
                    if(rowFrequencyMaps[i][val] || colFrequencyMaps[j][val] || blockFrequencyMaps[block_index][val])
                        return false;
                    rowFrequencyMaps[i][val] = colFrequencyMaps[j][val] = blockFrequencyMaps[block_index][val] = true;
                    }
                }
            }
        return true;
